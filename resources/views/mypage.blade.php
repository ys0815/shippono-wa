...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-lg text-amber-900 leading-tight">
            マイページ
        </h2>
    </x-slot>

    <div class="py-6 px-4 max-w-2xl mx-auto bg-main-bg min-h-screen">
        <!-- ユーザー情報ヘッダー -->
        ...
        <div class="mb-7">
            ...
            @forelse ($pets as $pet)
                <div class="bg-white rounded-lg p-4 mb-3 border">
                    <div class="flex items-start space-x-3">
                        <div class="w-12 h-12 bg-amber-100 border-2 border-amber-400 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden">
                            ...
                            @else
                                <span class="text-main-text text-sm font-bold">{{ mb_substr($pet->name,0,2) }}</span>
                            @endif
                        </div>
                        <div class="flex-1">
                            <h3 class="font-bold text-main-text">{{ $pet->name }}</h3>
                            <p class="text-xs text-main-text">
                            ...
                            @if($pet->shelter)
                                <p class="text-xs text-sub-text mt-1">保護施設: {{ $pet->shelter->name }}</p>
                            @endif
                ...
                <div class="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <h3 class="text-lg font-semibold text-main-text mb-2">まだ家族の一員を登録していませんね</h3>
                    <p class="text-sub-text mb-6">大切な家族の情報を登録して、みんなでシェアしましょう！</p>
            ...
            @if($pets->isNotEmpty())
                <a href="{{ route('mypage.pets') }}" class="block w-full text-center bg-gray-200 text-main-text py-3 rounded-lg font-medium hover:bg-gray-300 transition duration-200">
                    ペット一覧を見る→
                </a>
            @endif
        </div>
        ...
        <div class="mb-7">
            ...
            @empty
                <div class="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <h3 class="text-lg font-semibold text-main-text mb-2">まだ投稿はありません</h3>
                    <p class="text-sub-text mb-6">大切な家族との幸せな瞬間をシェアしてみませんか？</p>
                </div>
            @endforelse
            
            @if($recentPosts->isNotEmpty())
            <a href="{{ route('mypage.posts') }}" class="block w-full text-center bg-gray-200 text-main-text mb-4 py-3 rounded-lg font-medium hover:bg-gray-300 transition duration-200">
                すべての投稿を見る→
            </a>
        @endif
