<x-app-layout>
    <div class="min-h-screen bg-main-bg">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center py-4">
                    <div class="flex items-center">
                        ...
                        <h1 class="text-lg font-semibold text-amber-900">今日の幸せを投稿</h1>
                    </div>
                </div>
        ...
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <form action="{{ route('mypage.posts.gallery.store') }}" method="POST" enctype="multipart/form-data" class="space-y-10">
                ...
                <!-- メディアアップロード（画像・動画） -->
                <div>
                    <label for="media" class="block text-base font-medium text-main-text mb-2">写真・動画</label>
                    <div class="p-3 mb-3" style="background-color: #fefce8;">
                        <p class="text-sm" style="color: #a16207;">
                    ...
                    <div class="border-2 border-dashed border-sub-border rounded-lg p-6 text-center">
                        ...
                        <label for="media" class="cursor-pointer">
                            ...
                            <p class="mt-2 text-sm text-main-text">写真・動画を選択（最大2ファイル）</p>
    ...
    <script>
        ...
        // メディアプレビュー（画像・動画）: URL.createObjectURLで即時プレビューし、revokeでクリーンアップ
            ...
            if (input.files && input.files.length > 0) {
                ...
                Array.from(input.files).slice(0, 2).forEach((file, index) => {
                    ...
                    if (isVideo) {
                        ...
                        div.innerHTML = `
                            <img src="${objectUrl}" alt="プレビュー${index + 1}" loading="lazy" decoding="async" class="w-full h-32 object-cover rounded-lg" data-object-url="${objectUrl}">
                            <button type="button" onclick="removeMedia(this)" class="absolute top-2 right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm">×</button>
                        `;
                    }
                ...
                // ビデオのメタ読み込み後に軽いフェードインを適用（任意の視覚効果）
